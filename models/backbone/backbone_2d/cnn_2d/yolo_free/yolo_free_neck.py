"""
定义neck部分的SPP空间金字塔池化，其中有些SPP带有CSP交叉阶段差分 Cross Stage Partial设计
"""
import torch
...
# Spatial Pyramid Pooling  对同一个特征图，用不同尺寸的感受野进行池化，感受不同尺寸的特征
class SPP(nn.Module):
    """
    ...
    def __init__(self, in_dim, out_dim, expand_ratio=0.5, pooling_size=[5, 9, 13], norm_type='BN', act_type='relu'):
        ...
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type)  # 1x1卷积降低通道数量
        ...
        self.cv2 = Conv(inter_dim*(len(pooling_size) + 1), out_dim, k=1, act_type=act_type, norm_type=norm_type)  # 1x1卷积恢复通道数量
...
class SPPBlockCSP(nn.Module):
    ...
    def __init__(self,
        ...
        inter_dim = int(in_dim * expand_ratio)
        self.cv1 = Conv(in_dim, inter_dim, k=1, act_type=act_type, norm_type=norm_type)  # 1x1卷积，尺寸不变 只调节通道数，降低通道数
    ...
    def forward(self, x):
        ...
        x2 = self.cv2(x)
        x3 = self.m(x2)
        y = self.cv3(torch.cat([x1, x3], dim=1))  # 在通道维度上拼接

        return y


# Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher 更快的SPP，但是池化尺寸和中间通道数比较固定
