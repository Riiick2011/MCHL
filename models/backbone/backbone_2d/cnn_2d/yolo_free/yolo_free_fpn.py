...
class PaFPNELAN(nn.Module):
    def __init__(self, 
        ...
        # top dwon
        ## P5 -> P4
        self.cv1 = Conv(c5, int(256 * width), k=1, norm_type=norm_type, act_type=act_type)  # 只改变通道数
        ...
        if fpn_size == 'large' or fpn_size == 'huge':
            self.mp1 = DownSample(int(128 * width), act_type=act_type,
                                  norm_type=norm_type, depthwise=depthwise)  # 通道数量翻倍，空间尺寸减半
        elif fpn_size == 'tiny':
        ...
        self.head_conv_3 = Conv(int(512 * width), int(1024 * width), k=3, p=1,
                                act_type=act_type, norm_type=norm_type, depthwise=depthwise)
        # output proj layers
        if self.out_dim is not None:    # 统一三个层级的特征图的输出通道数
    ...
    def forward(self, features):
        ...
        out_feats = [c20, c21, c22]  # [P3, P4, P5]   尺寸28 14 7  通道数 256 512 1024
        
        # output proj layers  # 如果指定了输出的维度，则统一三个层级的特征图的输出通道数
        if self.out_dim is not None:
            out_feats_proj = []
            for feat, layer in zip(out_feats, self.out_layers):
                out_feats_proj.append(layer(feat))
            return out_feats_proj  # 列表包含3项，对应三个层级的特征图

        return out_feats  # 列表包含3项，对应三个层级的特征图
